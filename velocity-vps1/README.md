...
- `TAILSCALE_IP` - For the SFTP server, set this if you want to restrict the interfaces from which it can be reached (e.g. restricting from public access)

When updating Velocity versions, make sure to configure the variable `MINECRAFT_VERSION`, which is the Minecraft version being targeted, as well as the Java version within the variable `VELOCITY_IMAGE_NAME`, which should be a Java version supported by the desired Miencraft version.
