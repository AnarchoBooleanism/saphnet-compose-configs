...
All keys of a service should be listed in this order ([service-keys-order-rule](https://github.com/zavoloklom/docker-compose-linter/blob/main/docs/rules/service-keys-order-rule.md)): *`<<`*, *`extends`*, `image`, `build`, `container_name`, `depends_on`, `volumes`, `volumes_from`, `configs`, `secrets`, `environment`, `env_file`, `ports`, `networks`, `network_mode`, `extra_hosts`, `command`, `entrypoint`, `working_dir`, *`tty`*, `restart`, `healthcheck`, `logging`, `labels`, *`pid`*, `user`, `isolation`, `cap_add`, *`deploy`*, *`device_cgroup_rules`*, `devices`, `expose`, *`group_add`*, *`init`* *`security_opt`*, *`shm_size`*, `sysctls`, *`ulimits`* (all italicized keys are keys that were not originally in the list)
